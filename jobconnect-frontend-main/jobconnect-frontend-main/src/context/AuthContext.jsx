...
export const AuthProvider = ({ children }) => {
  ...
  const [user, setUser] = useState(() => {
    const stored = localStorage.getItem('gigconnect_user');
    return stored ? JSON.parse(stored) : null;
  });
  const [loading, setLoading] = useState(Boolean(localStorage.getItem('gigconnect_token')));
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        localStorage.setItem('gigconnect_user', JSON.stringify(res.data.user));
      })
      .catch(() => {
